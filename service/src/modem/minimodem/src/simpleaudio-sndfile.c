...
sa_sndfile_read( simpleaudio *sa, void *buf, size_t nframes )
{
    ...
    if (s == NULL) {
        assert(sa->format == SA_SAMPLE_FORMAT_FLOAT);

        // Read until we have all samples
        size_t bytes_to_read = nframes*sizeof(float);
        for (size_t bytes_read = 0; bytes_read < bytes_to_read;) {
                n = read(0, (char*)buf+bytes_read, bytes_to_read-bytes_read);
                if (n < 0) exit(1); //return -1;
                if (n == 0) return 0;
                bytes_read += n;
        }
...
sa_sndfile_write( simpleaudio *sa, void *buf, size_t nframes )
{
    ...
    if (s == NULL) {
        // Write until we have written all samples
        size_t bytes_to_write = nframes*sizeof(float);
        for (size_t bytes_written = 0; bytes_written < bytes_to_write;) {
                n = write(1, (char*)buf+bytes_written, bytes_to_write-bytes_written);
                if (n < 0) exit(1); //return -1;
                if (n == 0) return -1;
                bytes_written += n;
        }
		...
		char *app_name, char *stream_name )
{
    ...
    if (strcmp(path, "-") == 0) {
        ...
        sa->backend_framesize = 1 * sizeof(float);
        return 1;
    }
